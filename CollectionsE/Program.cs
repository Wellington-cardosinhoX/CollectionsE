...
RemoverMusicaPeloTitulo(rockNacional, "Que País é esse?");


ExibirMusicaAleatoria(rockNacional);
...
void ExibirMusicaAleatoria(Playlist playlist)
{
    ...
    {
        Console.WriteLine($"\nA música aleatória escolhida é: {musicaAleatoria.Titulo}");
    }
    else
    {
...
public class Musica : IComparable
{
    ...
    public int CompareTo(object? outraMusica)
    {
        if (outraMusica is null) return -1;

        if (outraMusica is Musica musica)
        {
...
public class Playlist : ICollection<Musica>
{
    ...
    {
        //lista.Sort((a, b) => a.Duracao.CompareTo(b.Duracao));
